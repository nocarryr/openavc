...
discovery:
  ...
  onvif:                           # ONVIF cameras; manufacturer disambiguates
    manufacturer: "Axis"
  hiqnet: true                     # HARMAN HiQnet on UDP 3804 — schema
                                   # accepts the opt-in but the engine
                                   # doesn't fire this probe yet
  symetrix: true                   # Symetrix ControlNet on UDP 49216 —
                                   # same: accepted, not yet wired
...
| `hiqnet` / `symetrix` | 2 | HARMAN HiQnet / Symetrix ControlNet broadcast probe opt-ins. Schema accepts these and registers signal-index rules; the engine does not currently fire either probe (deferred until validated against captures). Declaring them produces no Tier 2 evidence today. |
