...
provisioning-wizard mechanism; only ``kind: "command"`` is invocable today.
...
# provisioning wizard handled by the driver's run_setup_action().
ACTION_KINDS: tuple[str, ...] = ("command", "setup")
...
def resolve_device_actions(driver_info: dict[str, Any]) -> list[dict[str, Any]]:
