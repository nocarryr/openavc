...
def test_get_returns_301_with_https_url():
    resp = _client().get(
...
def test_post_returns_308_to_preserve_method():
    resp = _client().post(
...
def test_other_methods_use_308():
    for method, fn in (
        ("PUT", _client().put),
        ("PATCH", _client().patch),
        ...
        assert resp.status_code == 308, f"{method} should use 308"
